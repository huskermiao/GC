...
def main(mapfile, configfile, Ls=[]):
    ...
    gt_zeon = config.get('Section2', 'Letter_for_hete')
    gt_onon = config.get('Section2', 'Letter_for_homo2')
    gt_miss = config.get('Section2', 'Letter_for_missing_data')
    error_zeze = config.getfloat('Section3', 'error_rate_for_homo1')
    error_onon = config.getfloat('Section3', 'error_rate_for_homo2')
    error_zeon = abs(error_zeze- error_onon)
...
def cal_miss_rate(orig_seq, gt_miss):
    miss_num = orig_seq.count(gt_miss)
    missrate = miss_num/float(len(orig_seq))
    return missrate
