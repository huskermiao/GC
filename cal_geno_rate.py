...
def calhrate(map_file):
    ...
    for i,j in zip(sm_info,sm_seq):
        ...
        total_a += t_a_n
        total_b += t_b_n
        print 'X rate: %s'%(t_h_n/float(t_n))
        print 'A rate: %s'%(t_a_n/float(t_n))
        print 'B rate: %s'%(t_b_n/float(t_n))
    T = float(total_h+total_a+total_b)
    print 'For whole population:\nA: %s\nB: %s\nX: %s'%(total_a/T, total_b/T, total_h/T)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 2:
        ...
        print 'usage:\npython cal_hrate.py mapfile ho_a ho_b he_h'
